...
$timeFilter = $_GET['time'] ?? 'all'; // Фильтр по времени: all, week, month
$status = $_GET['status'] ?? 'all'; // Статус книги
$genre = $_GET['genre'] ?? 'all'; // Жанр книги
$page = $_GET['page'] ?? 1; // Текущая страница
$perPage = 10; // Количество книг на странице

// Определение временного интервала для фильтрации
$timeInterval = '';
if ($timeFilter === 'week') {
    $timeInterval = 'AND l.time >= NOW() - INTERVAL 1 WEEK';
} elseif ($timeFilter === 'month') {
    $timeInterval = 'AND l.time >= NOW() - INTERVAL 1 MONTH';
}

// Подготовка SQL-запроса с учетом фильтрации
$sql = "
    SELECT 
        b.id, 
        b.title, 
    ...
    LEFT JOIN like_book l ON b.id = l.book_id $timeInterval
    WHERE 1=1
";
...
$sql .= "
    GROUP BY b.id
    ORDER BY likes DESC, b.time DESC
...
<head>
    ...
    <title>Рейтинг книг - Read-Lit</title>
    <style>
        /* Стили для фильтров */
        .filters-container {
            display: none;
        ...
        .time-item.active, .time-item:hover {
            background: #343a40;
            color: white;
            border-color: #343a40;
        }
        ...
        <div class="content">
            ...
            <div class="filters-container" id="filtersContainer">
                <div class="card-body">
                    ...
                    <!-- Фильтр по времени -->
                    <span class="order-title">Период:</span>
                    <div class="time-filter">
                        <a class="time-item <?= $timeFilter === 'all' ? 'active' : '' ?>" 
                           href="?time=all&status=<?= $status ?>&genre=<?= $genre ?>&page=1">
                            За всё время
                        </a>
                        <a class="time-item <?= $timeFilter === 'week' ? 'active' : '' ?>" 
                           href="?time=week&status=<?= $status ?>&genre=<?= $genre ?>&page=1">
                            За неделю
                        </a>
                        <a class="time-item <?= $timeFilter === 'month' ? 'active' : '' ?>" 
                           href="?time=month&status=<?= $status ?>&genre=<?= $genre ?>&page=1">
                            За месяц
                        </a>
                    </div>
                    ...
                    <div role="group" class="btn-group btn-group-sm">
                        <a role="button" tabindex="0" 
                           href="?time=<?= $timeFilter ?>&status=all&genre=<?= $genre ?>&page=1" 
                           class="btn <?= $status === 'all' ? 'btn-dark' : 'btn-outline-secondary' ?>">
                            Все
                        </a>
                        <?php foreach ($statuses as $statusItem): ?>
                            <a role="button" tabindex="0" 
                               href="?time=<?= $timeFilter ?>&status=<?= $statusItem['id'] ?>&genre=<?= $genre ?>&page=1" 
                    ...
                    <div class="genre-filter">
                        <a class="genre-item <?= $genre === 'all' ? 'active' : '' ?>" 
                           href="?time=<?= $timeFilter ?>&status=<?= $status ?>&genre=all&page=1">
                            Все
                        </a>
                        <?php foreach ($genres as $genreItem): ?>
                            <a class="genre-item <?= $genre == $genreItem['id'] ? 'active' : '' ?>" 
                               href="?time=<?= $timeFilter ?>&status=<?= $status ?>&genre=<?= $genreItem['id'] ?>&page=1">
            ...
            <div class="books-list">
                <?php if (empty($books)): ?>
                    <div class="alert alert-info">Книги не найдены. Попробуйте изменить параметры фильтрации.</div>
                <?php else: ?>
                    <?php foreach ($books as $book): ?>
                        <div class="book-item">
                            ...
                            <div class="book-info">
                                ...
                                <p>Лайков: <span class="book-likes"><?= htmlspecialchars($book['likes']) ?></span></p>
            ...
            <?php if ($totalPages > 1): ?>
                <div class="pagination">
                    <?php if ($page > 1): ?>
                        <a href="?page=<?= $page-1 ?>&time=<?= $timeFilter ?>&status=<?= $status ?>&genre=<?= $genre ?>">
                            &laquo;
                        </a>
                    <?php endif; ?>
                    
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <a href="?page=<?= $i ?>&time=<?= $timeFilter ?>&status=<?= $status ?>&genre=<?= $genre ?>" 
                           class="<?= $i == $page ? 'active' : '' ?>">
                            <?= $i ?>
                        </a>
                    <?php endfor; ?>
